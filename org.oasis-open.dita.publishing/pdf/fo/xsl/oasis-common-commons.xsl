...
<!--                  <draft-comment>                                  -->
<!-- 31 Aug 2022 KJE: Added horizontal rule over <div> elements in     -->  
<!--                  draft comments                                   -->
<!-- 15 Sep 2022 KJE: Added rendering for when there is no             -->  
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="fo:block/@start-indent | fo:block/@font-size"
    mode="dropCopiedIds">
    ...
    <xsl:if test="$DRAFT = 'yes'">
      <fo:block xsl:use-attribute-sets="draft-comment">
        ...
        <fo:block xsl:use-attribute-sets="draft-comment__label">
          <xsl:choose>
            <xsl:when test="normalize-space(@author) != ''"> Comment by
                <xsl:value-of select="@author"/>
              <xsl:choose>
                <xsl:when test="normalize-space(@time) != ''"> on
  ...
  <!-- Add horizontal rule over <div> elements in draft comments -->
  <xsl:template match="draft-comment//div">
    ...
    <fo:page-sequence master-reference="body-sequence"
      ...
      <fo:flow flow-name="xsl-region-body">
        <fo:block xsl:use-attribute-sets="topic">
          ...
          <xsl:apply-templates
            select="*[contains(@class, ' topic/prolog ')]"/>
          ...
          <fo:block xsl:use-attribute-sets="topic.title">
            ...
            <xsl:attribute name="id">
              <xsl:value-of
                select="concat('_OPENTOPIC_TOC_PROCESSING_', generate-id())"
              />
            </xsl:attribute>
