...
<xsl:stylesheet version="2.0" xmlns:oa="http://www.oasis-open.org"
  ...
  <!-- Valid format values: xhtml, htmlhelp -->
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        ...
        <div class="footer">
          <p/>
          <xsl:copy-of select="document($FOOTER)//table"/>
        </div>
      </body>
      <xsl:value-of select="$newline"/>
    </html>
  ...
  <xsl:template match="*" mode="toc">
    ...
    <xsl:choose>
      <xsl:when test="normalize-space($thistitle) != ''">
        ...
        <p id="copy" name="copy" class="titlepageinfodescription">
